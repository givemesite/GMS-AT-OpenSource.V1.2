...
//
//////////////////////////////////////////////	
//	include('c:/php/WWW/Stock_Market/LOOP/sell_risks.php'); for documenting 
//	include('c:/php/WWW/Stock_Market/LOOP/buy_risks.php');  for documenting 
...
//get/save system historic data from stock table - helps find a stock sooner | time server

//calculate gain adv bias, from mysql stock table - improve thee gain and loss functions
...
function WATCH_STOCKS($stocks_watch_mode,$SERVER_DIR,$BACK_CALL,$emu,$sim,$in_loop_count){
...
//this will delete the old  stuff in the database		
		if ($the_ap == "am" && "9" == $the_hour && $the_min < "31" && $the_min >= "25"
){
...
if ($sum_adv > 0){
	...
	$DOWN_TREND = null;
		...
		if ($the_hour == 9 && $the_min <= 59 && $the_min >= 0 && $GAIN < 8 && $DOWN_TREND == TRUE){
		$GAIN = str_pad((round(map($the_min, 1, 59, 25 , 15),0,PHP_ROUND_HALF_UP)), 2, '0', STR_PAD_LEFT);
		//									default	25	 15		
		//											20	 13
		//											15	 10
			//										13	 08
		...
		if ($the_hour == 10 && $the_min <= 59 && $the_min >= 0 && $DOWN_TREND == TRUE){
			...
			$GAIN = str_pad((round(map($the_min, 1, 59, 15 , 8),0,PHP_ROUND_HALF_UP)), 2, '0', STR_PAD_LEFT);
			//											20	 11
		//												10   6
		//	$GAIN = "20";
		//}
...
//	$GAIN_Percent = 0.08;
		...
		echo "-------------------------------------------------------------------------------\n";
		echo"\n AVD / PADV: ".$old_advs."/".$padv." GAIN: ".$GAIN_Percent." OCG".$base_ocg . " FRQ". $base_frq . " ILC".$in_loop_count. "   COUNT". $base_count ."\n";//sleep(3);
		//reaction
		//find gains
		//even
		//Alpaca
