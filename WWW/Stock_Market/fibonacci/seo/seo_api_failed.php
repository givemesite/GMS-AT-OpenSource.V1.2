...
function TEST_VALS ($VALS){return strpos (  $VALS , '%' ,0 );return strpos ( $VALS , '+' ,0 );}
	...
	//this has some parts of the fail safes in it 
		 ...
		 if (!isset($row[1]) || $row[1]==''){
			 $$Clean_SYMB='';
				...
				$find_tradeing_symb_count=4;//start at placement 3 let it find it at 5
				while($find_tradeing_symb_count> 3 && $find_tradeing_symb_count < 5){// end placement 4 two bits to the right 

					//compile dataset with hyper links  
					//450 offset when live//update 1/14/19
					//18 offset updated 2/13/2019
								
					$A_Ticker_investing_table_count = $RENDER_id  + 18 + $find_tradeing_symb_count;$find_tradeing_symb_count--;
					echo $comp_name;
					echo "\n";
					//var_dump($aDataTableDetailHTML[0][3]['seo_data']);
					echo $aDataTableDetailHTML[0][3]['seo_data'][$A_Ticker_investing_table_count];
//echo $aDataTableDetailHTML[0][3]['seo_data'][$A_Ticker_investing_table_count];
					$Clean_SYMB=   investing_symb_lookup("",$aDataTableDetailHTML[0][3]['seo_data'][$A_Ticker_investing_table_count], $comp_name);
					usleep(650);
				
				}
				

				if ($Clean_SYMB<>"0" && $Clean_SYMB<>"" && $Clean_SYMB<>null){
				
				
				//$A_Ticker_investing_table_count = $RENDER_id  + 447;//448
		$conn = MYSQL_CONNECTOR($servername, $username, $password, $dbname);
				//$C_SYMB = 'test';
				$sql = "UPDATE stock SET trading_name = '". $Clean_SYMB. "' WHERE id = ".$row[0];
			//	$overRide_sql = "UPDATE stock SET trading_name = '0' WHERE id = ".$row[0];
			//is the first letter of the trading_name the same as the first one in the company  			
			if(mysqli_query($conn, $sql)){
							echo "Records were updated successfully. \n"."  Set Tradeing Name $Clean_SYMB $comp_name $price_now \n ";
							//restart the main loop
							continue(1);
				} else {
					echo "Error in SYMB. no match with uri \n";
							echo "ERROR: Could not update, unable to execute $sql. " . mysqli_error($conn) ."\n";
			}
			
			//close the door like a good dev
			mysqli_close($conn);			
...
//$servername, $username, $password, $dbname, $seo_api);
	...
	if ($is_a_stock== true && $set_stock==true){
		...
		$time = time();
			$sql = "UPDATE stock SET name='".$comp_name."',price='".$price_now."',day_low='".$day_low."',day_high='".$day_high."',CHANGE_PCT='".$CHANGE_PCT."',CHANGE_RATIO='".$CHANGE_RATIO."',CHANGE_VOL='".$CHANGE_VOL."',TIME='".$time."',PLACEMENT='".$RENDER_id."' WHERE id=".$UPDATE_ROW;
			if(mysqli_query($conn, $sql)){
							echo "Records were updated successfully. \n";
			} else {
							echo "ERROR: Could not update, unable to execute $sql. " . mysqli_error($conn) ."\n";
...
if ($NotAStock==true ){
 ...
 $time = time();
$sql = "INSERT INTO stock ( trading_name, name, price, day_low, day_high, CHANGE_RATIO, CHANGE_PCT, CHANGE_VOL, TIME, PLACEMENT, OLD_PLACEMENT, trading_name)
VALUES ( '$ticker_name', '$comp_name', '$price_now', '$day_low','$day_high','$CHANGE_RATIO','$CHANGE_PCT','$CHANGE_VOL', '$time', '$RENDER_id', '$RENDER_id', '$trading_name')";
if (mysqli_query($conn, $sql)) {
